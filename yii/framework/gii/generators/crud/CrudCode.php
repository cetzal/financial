...
class CrudCode extends CCodeModel
{
	...
	public function generateActiveField($modelClass,$column)
	{
		...
		elseif(stripos($column->dbType,'text')!==false)
			return "\$form->textArea(\$model,'{$column->name}',array('rows'=>6, 'cols'=>50, 'class'=>'form-control', 'placeholder'=>'texto'))";
		else
		{
			if(preg_match('/^(password|pass|passwd|passcode)$/i',$column->name))
			...
			if($column->type!=='string' || $column->size===null)
				return "\$form->{$inputField}(\$model,'{$column->name}', array('class'=>'form-control', 'placeholder'=>'texto'))";
			else
			{
				if(($size=$maxLength=$column->size)>60)
					$size=60;
				return "\$form->{$inputField}(\$model,'{$column->name}',array('size'=>$size,'maxlength'=>$maxLength, 'class'=>'form-control', 'placeholder'=>'texto'))";
			}
		}
	}
