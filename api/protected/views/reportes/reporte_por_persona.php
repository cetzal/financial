...
<?php 
  if (empty($modelmision)) {
  	echo "<h4><font color = 'red'>No se encontro niguna tarea para  esta persona</font></h4>";
  }
 ?>
 <h1>Tareas asignadas</h1>
<div class="contetList">
		<ul class="menu" id="accordion" >
			<?php  foreach ($modelmision as $key => $value) {  
				/*$datetime1 = date_create($value->fecha_final);
				$datetime2 = date_create(date("Y-m-d"));
				$interval = date_diff($datetime1, $datetime2);
				$diasatraso =  $interval->format('%a días');
*/
				?>
		     <li>
		        <div class="link"><h4><?php //echo "<font color='red'>".$diasatraso." de Atraso</font>"; ?></h4></div>
		          <ul class="submenu">
		            <li> 
		            	<?php  echo "<p>".$value['destareach']. "<p> fecha inicio ".$value['fecha_inicio']." -  fecha final ".$value['fecha_final']."<br>"; ?> 
